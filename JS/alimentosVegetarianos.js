...
function traerAlimentosRegulares() {
    let url = apiServer + "alimentoVegetariano/listar"
    fetch(url)
        .then((response) => response.json())
...
function traerAlimRegSelecc() {
    try {
        ...
        //Endpoint
        let url = apiServer + "alimentoVegetarianoSeleccionado/listar"
        fetch(url)
            .then((response) => response.json())
            .then((alimRegSelecc) => {
                alimRegSelecc.forEach(alimento => {
                    ...
                    tdImage.style = "width: 30px; text-align: center;";
                    imgAlimento.style = "margin-right: 10px;";
                    imgAlimento.src = "../../assets/img/alimentosVeg/" + alimento.fkIdAlimentoRegularSlcnn + ".png";
                    tdImage.appendChild(imgAlimento);
                    fila.appendChild(tdImage);
    ...
    try {
        ...
        for (let i = 0, food; food = foodSelections[i++];) {
            if (food.value !== "") {
                ...
                let url = apiServer + "alimentoVegetarianoSeleccionado/crear"
                fetch(url, {
                    method: 'POST',
                    body: JSON.stringify({
